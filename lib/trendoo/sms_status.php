...
function trendoo_get_message_status($order_id) {
	$post = new Trendoo_POST();
	$post->add_param('order_id',$order_id);
	$rp = $post->do_post(TRENDOO_MSG_STATUS_REQUEST);
	$res = $rp->get_result_array();
	$count = 0;
	if ($rp->isok) {
		while ($rp->go_next_line()) {
			$res[] = new Trendoo_SMS_STATUS($rp->next_string(), $rp->next_string(), $rp->next_string());
			$count++;
		}
	}
	$res['count'] = $count;
