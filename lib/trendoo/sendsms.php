...
function trendoo_removeScheduledSend($order_id) {
	$post = new Trendoo_POST();
	$post->add_param('order_id',$order_id);
	$post->do_post(TRENDOO_REMOVE_DELAYED_REQUEST);
	return true;
}
class Trendoo_SMS {
	var $order_id;
	...
	function validate() {
		...
		if (count($this->recipients) == 0) {
			...
			foreach ($this->recipients as $recipient) {
				if (!trendoo_is_valid_international($recipient)) {
					$this->problem = 'invalid recipient: '.$recipient;
					return false;
				}
			}
		}
		if (!trendoo_sms_type_valid($this->sms_type)) {
			$this->problem = 'invalid SMS type: '.$this->sms_type;
			return false;
		}
		if (trendoo_sms_type_has_custom_tpoa($this->sms_type)) {
			if (!trendoo_is_valid_tpoa($this->sender)) {
	...
	function set_scheduled_delivery($timestamp) {
		$this->scheduled_delivery = strftime(TRENDOO_DATE_TIME_FORMAT,$timestamp);
	}
	...
	function send() {
		...
		$rp = $post->do_post(TRENDOO_SEND_SMS_REQUEST);
