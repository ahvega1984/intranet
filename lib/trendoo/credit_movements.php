...
function trendoo_get_subaccount_credits($login) {
	$post = new Trendoo_POST();
	$post->add_param('op','GET_CREDITS');
	$post->add_param('subaccount',$login);
	$rp = $post->do_post(TRENDOO_SUBACCOUNTS_REQUEST);
...
function trendoo_get_subaccount_purchases($login) {
	$post = new Trendoo_POST();
	...
	if ($rp->isok) {
		while ($rp->go_next_line()) {
			$cm = new Trendoo_CREDIT_MOVEMENT();
