$( document ).ready(function() {
	$.post( "estadisticas/estadisticas.php", { "id" : 'accesos' }, null, "json" )
	    .done(function( data, textStatus, jqXHR ) {
	       ...
	       if (data.accesos_tabla.length > 0) {
	           $('#stats-accesos-modal').html('<div style="height: 350px; overflow: scroll;"><table class="table table-condensed table-hover table-striped"><thead><tr><th>Profesor/a</th><th>Departamento</th></tr></thead><tbody id="accesos_tabla"></tbody></table></div>');
	...
	$.post( "estadisticas/estadisticas.php", { "id" : 'visitas' }, null, "json" )
	    .done(function( data, textStatus, jqXHR ) {
	       ...
	       if (data.visitas_tabla.length > 0) {
	           $('#stats-visitas-modal').html('<table class="table table-condensed table-hover table-striped"><thead><tr><th>Alumno/a</th><th>Unidad</th><th>Tutor</th></tr></thead><tbody id="visitas_tabla"></tbody></table>');
	           
	           $.each(data.visitas_tabla, function(i, item) {
	          	 $('#visitas_tabla').append('<tr onclick="window.location.href=\'admin/infotutoria/infocompleto.php?id=' + item.idvisita + '\'" style="cursor: pointer; font-size: 0.9em;"><td nowrap>' + item.alumno + '</td><td>' + item.unidad + '</td><td nowrap>' + item.tutor + '</td></tr>');
	           });
	       }
	       else {
	...
	$.post( "estadisticas/estadisticas.php", { "id" : 'mensajes' }, null, "json" )
	    .done(function( data, textStatus, jqXHR ) {
