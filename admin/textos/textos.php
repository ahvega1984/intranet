  ...
  while ($row = mysqli_fetch_array($textos))
   {
		  ...
		  <td>$row[8]</td>";
		   if(stristr($_SESSION['cargo'],'1') == TRUE OR (stristr($_SESSION['cargo'],'4') == TRUE and $_SESSION['dpt'] == $row[0]))
{
	echo "<td nowrap><a href='editextos.php?id=$row[6]'><i class='fas fa-pencil-alt' title='Editar'> </i> </a>&nbsp;&nbsp;<a href=deltextos.php?id=$row[6] style='color:brown;' data-bb='confirm-delete'><i class='far fa-trash-alt' title='Borrar' > </i></a></td>";
			 }
