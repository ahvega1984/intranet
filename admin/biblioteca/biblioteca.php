...
if (!($autor == "" and $titulo0 == "" and $editorial == "")) {
	...
	if (mysqli_num_rows($result) > 0) {
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			printf ("<tr><td class='text-success'>%s</td><td>%s</td><td>%s</td><td><a href='biblioteca.php?idfondo=$id&autor=$autor&titulo0=$titulo0&editorial=$editorial' data-bs='tooltip' title='Detalles'><span class='fas fa-search fa-fw fa-lg'></span></a></td></tr>", $row[1], $row[2], $row[3]);
		}
		echo "</table>";
	}
	else {
