...
if (isset($_POST['enviar_datos'])) {
	...
	mysqli_query($db_con,"delete from informe_pendientes_alumnos where id_informe = '".$id_informe."'");
	foreach ($_POST as $clave => $val) {
		...
		$result = mysqli_query($db_con,"INSERT into informe_pendientes_alumnos (id_informe,id_contenido,claveal,actividades) VALUES ('$id_informe','$id_contenido','$claveal','$observaciones')");
			if (! $result) {
				...
				$msg_acierto = "Se han guardado correctamente los datos del informe sobre la evaluación de pendientes para los alumnos afectados";
			}
		}
	}
		$id_informe = $_POST['id_informe'];
...
$materias = mysqli_query($db_con,"select codigo, nombre, curso from materias where grupo = '$grupo' and nombre = (select asignatura from informe_pendientes where id_informe = '$id_informe') and abrev not like '%\_%'");
...
?>
	...
	<div class="container">
		...
		<div class="page-header">
			<h2>Informe individual para la evaluación de alumnos con materias pendientes <small> <br>Selección de alumnos y actividades</small></h2>
		</div>
