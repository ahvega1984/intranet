...
if(isset($_POST['actividad'])){
	...
	$_SESSION['mesas']['id'] = trim($exp_act[5]);
}
...
function al_con_nie($db_con, $var_nie, $var_grupo) {
	$result = mysqli_query($db_con, "SELECT unidad, CONCAT(nombre, ', ', apellidos) AS alumno FROM alma WHERE claveal='".$var_nie."' ORDER BY apellidos ASC, nombre ASC LIMIT 1");
	$row = mysqli_fetch_array($result);
	mysqli_free_result($result);
	
	if ($row['alumno'] != ", ") {
		if (mb_strstr($var_grupo,'+')){return($row['alumno'].', '.$row['nombre'].' ('.$row['unidad'].')');}
		else {return($row['alumno'].', '.$row['nombre']);}
	}
	else {
		return('');
	}
...
if (isset($_POST['listOfItems'])){
	$result_update = mysqli_query($db_con, "UPDATE puestos_alumnos_tic SET monopuesto='".$_SESSION['mesas']['monopuesto']."',puestos='".$_POST['listOfItems']."' WHERE actividad='".$actividad."'");
...
if (! mysqli_num_rows($result)) {
	$result_insert = mysqli_query($db_con, "INSERT INTO puestos_alumnos_tic (actividad, puestos, monopuesto) VALUES ('".$actividad."', '','0')");
...
?>
	...
	<div class="container">
		...
		<div class="page-header">
			<h2>Grupo <?php echo $grupo; ?> <small>Asignación de mesas TIC</small></h2>
			<h4 class="text-info">Profesor/a: <?php echo nomprofesor($profesor); ?>  (Asignatura: <?php echo $asig; ?> )</h4>
		</div>
			...
			<div class="col-sm-12">
				...
				<div class="hidden-print">
					<form id="myForm" name="myForm" method="post" action="" onsubmit="saveDragDropNodes()">
						...
						<a class="btn btn-default" href="sel_grupo_mesas_tic.php">Volver</a>
					</form>
				</div>
