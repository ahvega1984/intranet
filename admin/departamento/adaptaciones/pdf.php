...
if (isset($_GET['id'])) {
	...
	$result = mysqli_query($db_con, "SELECT apellidos, nombre, adaptaciones.unidad, materia,  texto, adaptaciones.fecha, departamento FROM adaptaciones, alma WHERE alma.claveal= adaptaciones.alumno and id = '$id'");
}
...
if ($result) {
	...
	$dompdf->render();
	if ($sonVarias > 2) {
		$dompdf->stream("Adaptacion cuuricular de $nombre $apellidos ($materia).pdf", array("Attachment" => 0));
	}
	else {
