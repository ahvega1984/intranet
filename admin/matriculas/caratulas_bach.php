...
for($i=1;$i<2;$i++){
	...
	$MiPDF->Ln ( 15 );
	$titulo1 = utf8_decode("SOLICITUD DE MATRÍCULA EN ".$n_curso."º DE BACHILLERATO");
	...
	$MiPDF->Cell(28,5,utf8_decode($config['centro_provincia']),1,0,'C');
	$MiPDF->Ln ( 8 );
	$MiPDF->Cell(168,5,utf8_decode("CORREO ELECTRÓNICO DE CONTACTO"),0,0,'C');
	$MiPDF->Ln ( 5 );
	...
	$MiPDF->Ln ( 5 );
	$MiPDF->Cell(76,5,utf8_decode($colegio),1,0,'C');
	$MiPDF->Cell(46,5,utf8_decode($config['centro_localidad']),1,0,'C');
	$MiPDF->Cell(46,5,utf8_decode($config['centro_codigo']),1,0,'C');
	$MiPDF->Ln ( 10 );
	...
	$MiPDF->Cell(70,5,utf8_decode("CURSO EN QUE SE MATRICULA"),0,0,"C");
	...
	if ($curso=="1BACH") {
	$MiPDF->Cell(80,5,$n_curso.utf8_decode("º DE BACH. ( ").utf8_decode($mod_registro)." )",1,0,'C');
	$MiPDF->MultiCell(88,5,utf8_decode($bil.$opt),1);
	$MiPDF->Ln ( 2 );
	}

	if ($curso=="2BACH") {
	$MiPDF->Cell(80,5,$n_curso.utf8_decode("º DE BACH. ( ").utf8_decode($mod_registro)." ".utf8_decode($optativa2)." )",1,0,'C');
	$MiPDF->MultiCell(88,5,utf8_decode($bil.$opt2b),1);
	$MiPDF->Ln ( 5 );
	...
	if(!$repetidor){
	$MiPDF->Cell(165,5,utf8_decode("MODALIDAD Y OPTATIVAS DE 1º BACHILLERATO"),0,0,"C");
	$MiPDF->Ln ( 5 );
	$MiPDF->Cell(80,5,"".utf8_decode($mod_registro)." ",1,0,'C');
	$MiPDF->MultiCell(88,5,utf8_decode($bil.$optativa1),1);
	$MiPDF->Ln ( 2 );
	}
	}
  }
