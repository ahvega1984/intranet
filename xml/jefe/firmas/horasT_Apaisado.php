...
while ($row_jornada = mysqli_fetch_array($result_jornadas)) {
	...
	$horas_jornada = array();
	$result_horas_jornada = mysqli_query($db_con, "SELECT `tramo`, `hora`, `hora_inicio`, `hora_fin` FROM `tramos` WHERE `idjornada` = '".$row_jornada['idjornada']."' and hora > 6 ORDER BY `horini` ASC, `horfin` ASC");
	...
	for ($diasemana = 1; $diasemana < 6 ; $diasemana++) {
		...
		//$MiPDF->Cell(78, 5, 'Horario: '.$num_horario." de ".$turno, 0, 0, 'C');
		$MiPDF->Cell(78, 5, '[Turno de '.$turno.']', 0, 0, 'C');
		
		$dia = strtotime($fechaInicio. ' + '.($diasemana - 1).' days');
		$nombredia = $arrayDias[date('w', $dia)];	

		$MiPDF->Cell(94, 5, $nombredia.'  '.date('d/m/Y',$dia), 0, 1, 'R');
		$MiPDF->Ln(5);
		...
		$titulos = array();
		array_push($titulos, "\nProfesor/a");
		array_push($ancho_columnas, 60);
		array_push($titulos, "Firma\nEntrada");
		array_push($ancho_columnas, 40);
		...
		while ($row_profesores = mysqli_fetch_array($result_profesores)) {
			...
			foreach ($horas_jornada as $hora_jornada) {
				$result_horario = mysqli_query($db_con, "SELECT `a_asig`, `a_aula`, `a_grupo` FROM `horw` WHERE `prof` = '".$row_profesores['profesor']."' AND `dia` = '".date('w', $dia)."' AND `hora` = '".$hora_jornada['hora']."' AND `c_asig` NOT IN (SELECT `idactividad` FROM `actividades_seneca`) LIMIT 1");
				$asignatura_tramo = "";
				$aula_tramo = "";
				$grupo_tramo = "";
				while ($row_horario = mysqli_fetch_array($result_horario)) {
					...
					$aula = "";
					$grupo = "";
					if (strlen($row_horario['a_asig']) > 9) {
						$abrev = substr($row_horario['a_asig'], 0, 9);
					}
					else {
						$abrev = $row_horario['a_asig'];
					}
					if (strlen($row_horario['a_aula']) > 9) {
						$aula = substr($row_horario['a_aula'], 0, 9);
					}
					else {
						$aula = $row_horario['a_aula'];
					}
					if (strlen($row_horario['a_grupo']) > 9) {
						$grupo = substr($row_horario['a_grupo'], 0, 9);
					}
					else {
