...
foreach ($unidad_asignatura_evento as $grupo_cal) {
	...
	if (mysqli_num_rows($chk_exam) && (isset($config['calendario']['prefExamenes']) && $config['calendario']['prefExamenes'] == 0) && strstr($_SESSION['cargo'], "1") == FALSE && $_SESSION['user_admin'] !== 1) {
		header('Location:'.'http://'.$config['dominio'].'/intranet/calendario/index.php?mes='.$_GET['mes'].'&anio='.$_GET['anio'].'&msg_cal=10');
		exit();
	}

	$chk_actividad = mysqli_query($db_con,"select * from calendario where categoria = '2' and fechaini <= '$fecha_extra_ini' and fechafin >= '$fecha_extra_fin' and unidades like '%$gr_cal%'");
	if (mysqli_num_rows($chk_actividad) && (isset($config['calendario']['prefActividades']) && $config['calendario']['prefActividades'] == 0) && strstr($_SESSION['cargo'], "1") == FALSE && $_SESSION['user_admin'] !== 1) {
...
foreach ($unidades_evento as $grupo_cal1) {
	...
	$chk = mysqli_query($db_con,"select * from calendario where categoria = '2' and fechaini <= '$fecha_extra_ini' and fechafin >= '$fecha_extra_fin' and unidades like '%$grupo_cal1%'");
		if (mysqli_num_rows($chk) && (isset($config['calendario']['prefActividades']) && $config['calendario']['prefActividades'] == 0) && strstr($_SESSION['cargo'], "1") == FALSE && $_SESSION['user_admin'] !== 1) {
